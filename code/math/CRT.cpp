...
pll sol(tuple<ll, ll, ll> c){ //requires inv, diophantine
  ll a = get<0>(c), x1 = get<1>(c), m = get<2>(c),
    d = __gcd(a,m);
  if(d == 1) return pll(MOD(x1 * inv(a, m), m), m);
  else return x1 % d ? 
    pll({-1LL, -1LL}) : sol(make_tuple(a/d, x1/d, m/d));
}
// returns: (sol, lcm)
pair<ll, ll> crt(vector< tuple<ll, ll, ll> > &cond) {
  ...
  for(auto &t: cond){
    ...
    if(m1 == m2)continue;
    ll k = diophantine(m2, -m1, x1-x2).fi.se,
      l = m1 * (m2 / __gcd(m1, m2));
    x1 = MOD((__int128_t)m1*k + x1, l); m1 = l;
  }
  return sol(make_tuple(1, x1, m1));
} //cond[i] = {ai, bi, mi} ai*xi = bi (mi); 
// assumes lcm fits in ll